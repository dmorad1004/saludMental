...
class VacunappSupabaseUser extends BaseAuthUser {
  VacunappSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> vacunappSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = VacunappSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
